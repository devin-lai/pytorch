...
class CommonTemplate:
    ...
    def test_split_reduction_preserves_block_ptr_multidim(self):
        # When a multi-dim tensor is fully reduced (e.g. sum() with no
        # dim), the split can produce a block_size misaligned to the
        # innermost dimension.  This should fall back to
        # create_multilayer_existing_ranges to preserve block_ptr.
        M = 70376
        N = 96
        x = torch.randn(M, N, device=self.device)
        ...
        # The first kernel should use block_ptr (tl.make_block_ptr),
        # not scalar loads with modular indexing.
        first_kernel = code[0]
        self.assertTrue(
            "make_block_ptr" in first_kernel,
            "Expected block_ptr in split reduction kernel, but got scalar loads. "
            "The split likely produced a block_size misaligned to the inner dim.",
        )
